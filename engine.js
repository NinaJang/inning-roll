...
const { SPECIALTIES } = require('./teams');
...
// 삼진/볼넷/땅볼안타 같은 "판정 다툴 여지가 없는" 결과는 대상에서 뺀다.
const REVIEWABLE_CODES = new Set(['SB', 'CS', 'PO', 'HR', 'F']);
...
function createGame(awayTeam, homeTeam, maxInnings = 9) {
  return {
    maxInnings,
    inning: 1,
    half: 'top',
    ...
    challenges: { away: 2, home: 2 },       // KBO와 동일: 성공하면 유지, 실패하면 소진
    strategyUses: { away: 3, home: 3 },     // 감독 작전 지시 - 경기당 팀당 3회
    pendingModifier: null,                  // 감독 지시(부스트형)가 다음 타석에 걸어둔 임시 보정
  };
}

function battingTeam(state) {
  return state.half === 'top' ? 'away' : 'home';
}

function fieldingTeam(state) {
  return state.half === 'top' ? 'home' : 'away';
}

function hasRunner(state) {
...
function applyWalk(bases, batter) {
  const [r1, r2, r3] = bases;
  let runs = 0;
  const next = [batter, bases[1], bases[2]];
  if (r1) {
    if (r2) {
      if (r3) runs += 1;
      next[2] = r2;
...
function applyAdvance(bases, hitBases, batter) {
  let runs = 0;
...
function applyDoublePlay(bases) {
  if (!bases[0]) return { bases: bases.slice(), extraOut: false };
...
function applyStealOutcome(state, fromIdx, runner, success) {
  ...
  if (success) {
    bases[toIdx] = runner;
    state.bases = bases;
    return { code: 'SB', label: `도루 성공 - ${runner.name} (${toName})` };
  }
  state.bases = bases;
  state.outs += 1;
  return { code: 'CS', label: `도루 실패 - ${runner.name} 아웃 (${toName})` };
}

function makeEvent(state, extra) {
  return Object.assign({
    inning: state.inning,
    half: state.half,
    team: battingTeam(state),
    outsAfter: Math.min(state.outs, 3),
    basesAfter: state.bases.slice(),
    halfEnded: false,
    gameOver: false,
    runs: 0,
    isChaos: false,
...
function checkWalkoff(state, event) {
...
function endHalfInning(state, event) {
...
function tryChaosEvent(state) {
  if (!hasRunner(state)) return null;
  const snapshot = snapshotState(state);

  let stealFromIdx = null;
  ...
  const wpChance = 0.02 + (pitcherSpec && pitcherSpec.extraWildPitchChance ? pitcherSpec.extraWildPitchChance : 0);
  const bkChance = 0.02;
  const poChance = 0.03;
  const sbChance = stealFromIdx !== null
    ? 0.08 + (stealSpec && stealSpec.stealAttemptBonus ? stealSpec.stealAttemptBonus : 0)
    : 0;
  ...
  if (roll < cursor) {
    ...
    return finalizeEvent(state, makeEvent(state, { code: 'WP', label: `${pitcher.name} 투수의 폭투`, runs, isChaos: true, snapshot }));
  }
  ...
  if (roll < cursor) {
    ...
    state.bases = [null, b1, b2];
    state.score[team] += runs;
    return finalizeEvent(state, makeEvent(state, { code: 'BK', label: `${pitcher.name} 투수의 보크`, runs, isChaos: true, snapshot }));
  }
  ...
  if (roll < cursor) {
    ...
    return finalizeEvent(state, makeEvent(state, {
      code: 'PO', label: `견제사 - ${runnerOut.name} (${baseName})`, isChaos: true,
      snapshot, meta: { runner: runnerOut },
    }));
  }

  cursor += sbChance;
  if (roll < cursor && stealFromIdx !== null) {
    const successChance = 0.70 + (stealSpec && stealSpec.stealSuccessBonus ? stealSpec.stealSuccessBonus : 0);
...
function applyBatterOutcome(state, code, batter, opts = {}) {
  ...
  switch (code) {
    ...
    case 'G': {
      ...
      if (!opts.noFlavor && canHitRunner && Math.random() < 0.08) {
        ...
        state.bases = bases;
        outsAdded = 1;
        label = `타구에 맞은 ${hitRunner.name} 아웃 (${baseName}), 타자는 1루 출루`;
      } else {
        outsAdded = 1;
      }
      break;
    ...
    case 'BB': {
      if (!opts.noFlavor) {
        ...
        const hbpChance = 0.20 + (pitcherSpec && pitcherSpec.extraHbpChance ? pitcherSpec.extraHbpChance : 0);
...
function applyPendingModifier(mod, code) {
...
function resolveBatterRoll(state) {
  const snapshot = snapshotState(state);
  ...
  let code = OUTCOME_TABLE[sum];
  let feat = null;

  // 1) 타자 특기: 아웃을 더 좋은 결과로 격상
  const isOut = code === 'K' || code === 'F' || code === 'G' || code === 'DP';
  if (isOut && batterSpec && batterSpec.outUpgradeChance && Math.random() < batterSpec.outUpgradeChance) {
    code = batterSpec.outUpgradeTo;
    feat = `${batter.name}의 ${batterSpec.label}`;
  }

  // 2) 투수 특기: 출루성 결과를 다시 억누름
  const isHitLike = code === '1B' || code === '2B' || code === '3B' || code === 'HR' || code === 'BB';
  if (pitcherSpec && pitcherSpec.hitDowngradeChance && isHitLike && Math.random() < pitcherSpec.hitDowngradeChance) {
    code = pitcherSpec.hitDowngradeTo;
    feat = `${pitcher.name}의 ${pitcherSpec.label}`;
  } else if (pitcherSpec && pitcherSpec.hrSuppressChance && code === 'HR' && Math.random() < pitcherSpec.hrSuppressChance) {
    code = 'G';
    feat = `${pitcher.name}의 ${pitcherSpec.label}`;
  }
  ...
  return finalizeEvent(state, makeEvent(state, {
    d1, d2, sum, code, label: finalLabel, runs, batterName: batter.name, snapshot,
  }));
}

// 비디오 판독. 성공 확률(45%)로 판정을 뒤집는다: 상태를 판정 직전으로 되돌린 뒤
// 반대 결과를 강제로 적용한다. 실패하면 원래 결과 그대로 유지.
function attemptChallenge(state, event) {
  ...
  if (event.code === 'SB' || event.code === 'CS') {
    ...
    newEvent = finalizeEvent(state, makeEvent(state, {
      code: 'PO_SAFE', label: `[판독 번복] ${event.meta.runner.name} 세이프 처리`, isChaos: true, isReview: true,
    }));
  } else if (event.code === 'HR' || event.code === 'F') {
    const batter = getCurrentBatter(state);
    const wantCode = event.code === 'HR' ? 'F' : 'HR';
    ...
    newEvent = finalizeEvent(state, makeEvent(state, {
      code: wantCode, label: `[판독 번복] ${label}`, runs, batterName: batter.name, isReview: true,
    }));
  }
...
const OFFENSE_STRATEGIES = [
  {
    ...
    execute: (state) => {
      const batter = getCurrentBatter(state);
      const team = battingTeam(state);
      const code = Math.random() < 0.8 ? 'SAC_OK' : 'SAC_FAIL';
      const { label, runs, outsAdded } = applyBatterOutcome(state, code, batter);
      state.outs += outsAdded;
      state.score[team] += runs;
      advanceBattingIndex(state, team);
      const event = finalizeEvent(state, makeEvent(state, {
        code, label: `[감독 전략: 번트] ${label}`, runs, batterName: batter.name, isStrategy: true,
      }));
...
const DEFENSE_STRATEGIES = [
  {
    ...
    execute: (state) => {
      ...
      advanceBattingIndex(state, team);
      const event = finalizeEvent(state, makeEvent(state, {
        code: 'BB', label: `[감독 전략: 고의 4구] ${label}`, runs, batterName: batter.name, isStrategy: true,
      }));
      return { plays: [event], continueToBatter: false, skipChaos: true };
    },
  },
    ...
    execute: (state) => {
      ...
      if (Math.random() < 0.5) {
        ...
        event = finalizeEvent(state, makeEvent(state, {
          code: 'PO', label: `[감독 전략: 총력 견제] ${runner.name} 아웃 (${baseName})`, isChaos: true, isStrategy: true,
          meta: { runner },
        }));
      } else {
        event = finalizeEvent(state, makeEvent(state, {
          code: 'PO_MISS', label: `[감독 전략: 총력 견제] 견제 실패 - ${runner.name} 생존`, isChaos: true, isStrategy: true,
        }));
      }
      return { plays: [event], continueToBatter: true, skipChaos: true };
    },
...
function useStrategy(state, side) {
  ...
  return Object.assign({ chosenKey: choice.key, chosenLabel: choice.label }, result);
}

// 한 타자의 턴을 진행한다 (챌린지/전략 없이 단순 진행 — 시뮬레이션용).
function playAtBat(state, options = {}) {
...
module.exports = {
  OUTCOME_TABLE, OUTCOME_LABELS, rollDice, createGame, playAtBat,
  getCurrentBatter, battingTeam, fieldingTeam,
  tryChaosEvent, resolveBatterRoll, useStrategy, attemptChallenge,
  isReviewable, challengeSide,
};
